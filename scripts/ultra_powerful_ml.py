...
def train_pytorch_model(
    ...
    class_weights=None, patience=15, model_name="Model"
):
...
def train_and_evaluate_deep_models(
    ...
    if use_sequences:
        ...
        X_seq, y_seq, subj_seq = create_sequences(X, y, subject_ids, seq_length=10, stride=5)
        ...
        for train_idx, test_idx in cv_splits:
            ...
            seq_cv_splits.append((
                np.where(seq_train_mask)[0],
                np.where(seq_test_mask)[0]
            ))
    ...
    for model_name, model_factory in models_config:
        ...
        for fold, (train_idx, test_idx) in enumerate(cv_use, 1):
            ...
            best_val_acc = train_pytorch_model(
                ...
                device=device, class_weights=class_weights, patience=15,
...
def main():
    ...
    parser.add_argument("--use-sequences", action="store_true", help="Use temporal sequences for LSTM/GRU")
    ...
    def create_subject_level_cv_splits(subject_ids, y_per_subject, n_splits=5, random_state=42):
    ...
    cv_splits = create_subject_level_cv_splits(subject_ids, y_per_subject, args.n_folds, args.seed)
    ...
    traditional_results['XGBoost_Optimized'] = {
        ...
        'mean_f1': f1_score(all_y_true, all_y_pred, average='weighted'),
        'confusion_matrix': cm.tolist(),
        'classification_report': classification_report(all_y_true, all_y_pred, output_dict=True, zero_division=0)
    }
    
    print(f"  ✓ Mean Accuracy: {traditional_results['XGBoost_Optimized']['mean_accuracy']:.4f} ± {traditional_results['XGBoost_Optimized']['std_accuracy']:.4f}")
    ...
    sorted_models = sorted(all_results.items(), key=lambda x: x[1]['mean_accuracy'], reverse=True)
    
    for rank, (model_name, res) in enumerate(sorted_models, 1):
        print(f"{rank}. {model_name}: {res['mean_accuracy']:.4f} ± {res['std_accuracy']:.4f}")
