...
def analyze_sensor_data(data_dir: str, label_strategy: str = "ordinal_3class"):
    ...
    acc_features = [
        f
        for f in feature_names
        if "ACC" in f or "accel" in f.lower() or "std" in f.lower()
    ]
    eda_features = [
        f
        for f in feature_names
        if "EDA" in f or "gsr" in f.lower() or "skin" in f.lower()
    ]
    temp_features = [f for f in feature_names if "temp" in f.lower() or "TEMP" in f]
    other_features = [
        f
        for f in feature_names
        if f not in ecg_features + acc_features + eda_features + temp_features
    ]
    ...
    for feat in ecg_features:
        ...
        vals = all_X[:, idx]
        print(
            f"  - {feat:30s}: mean={np.mean(vals):10.2f}, std={np.std(vals):10.2f}, "
        ...
        subject_stats.append(
            {
                ...
                "classes": dict(
                    zip(subject_classes[0].astype(int), subject_classes[1])
                ),
            }
        )
    ...
    ax1.hist(
        samples_per_subject, bins=30, color="steelblue", alpha=0.7, edgecolor="black"
    )
    ax1.set_xlabel("Samples per Subject")
    ax1.set_ylabel("Number of Subjects")
    ax1.set_title("Distribution of Samples per Subject")
    ax1.axvline(
        np.mean(samples_per_subject),
        color="red",
        linestyle="--",
        label=f"Mean: {np.mean(samples_per_subject):.1f}",
    )
    ax1.legend()
    ax1.grid(alpha=0.3)
    ...
    for i, (split_name, split_y, color) in enumerate(
        zip(splits, split_data, split_colors)
    ):
    ...
    ax3.set_xticklabels(
        [f[:15] for f in top_features_names], rotation=45, ha="right", fontsize=8
    )
    ...
    for i in range(3):
        ...
        for cls in unique:
            ...
            ax.hist(
                class_data, bins=30, alpha=0.5, label=f"Class {int(cls)}", density=True
            )

        ax.set_xlabel("Value")
        ax.set_ylabel("Density")
        ax.set_title(
            f"{feat_name[:25]}..." if len(feat_name) > 25 else feat_name, fontsize=10
        )
    ...
    if len(ecg_features) > 0:
        ...
        df_ecg = pd.DataFrame(
            {"Feature": ecg_labels, "Class": ecg_classes, "Value": ecg_data}
        )
    ...
    if len(acc_features) > 0:
        ...
        df_acc = pd.DataFrame(
            {"Feature": acc_labels, "Class": acc_classes, "Value": acc_data}
        )
    ...
    plt.suptitle(
        "SWEET Dataset - Comprehensive Sensor & Feature Analysis",
        fontsize=16,
        fontweight="bold",
    )
