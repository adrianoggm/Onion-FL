...
def create_subject_level_cv_splits(data_dict, n_folds=5, seed=42):
    ...
    for subj in subjects:
        subj_labels = [l for s, l in zip(data_dict['subjects'], data_dict['labels']) if s == subj]
    ...
    for fold_idx in range(n_folds):
        ...
        train_indices = [i for i, s in enumerate(data_dict['subjects']) if s not in test_subjects]
        test_indices = [i for i, s in enumerate(data_dict['subjects']) if s in test_subjects]
...
class VeryWideDeepMLP(nn.Module):
    ...
    def __init__(self, input_dim, num_classes=3, hidden_dims=[1024, 1024, 512, 512, 256, 256, 128, 128], dropout=0.4):
        super().__init__()
...
class PyramidMLP(nn.Module):
    ...
    def __init__(self, input_dim, num_classes=3, max_width=2048, min_width=64, depth=12, dropout=0.3):
        super().__init__()
...
def train_pytorch_model(model, train_loader, val_loader, device, epochs=200, lr=0.001, patience=30):
...
def evaluate_extreme_models(X_train_full, y_train_full, subjects_full, cv_splits, device):
    ...
    if HAS_XGBOOST:
        ...
        for name, config in xgb_configs.items():
            print(f"\n📊 {name}:")
            print(f"   Config: depth={config['max_depth']}, n_estimators={config['n_estimators']}, lr={config['learning_rate']}")
    ...
    mlp_configs = {
        ...
        'VeryWide_Deep_MLP': {
            ...
            'kwargs': {'hidden_dims': [1024, 1024, 512, 512, 256, 256, 128, 128], 'dropout': 0.4},
            'epochs': 200,
            'lr': 0.001,
            'batch_size': 64
        },
        ...
        'Pyramid_MLP_16layers': {
            'model_class': PyramidMLP,
            'kwargs': {'max_width': 1024, 'min_width': 32, 'depth': 16, 'dropout': 0.35},
            'epochs': 200,
            'lr': 0.0007,
            'batch_size': 64
        }
...
def main():
    ...
    print(f"✓ Dataset loaded: {len(X_train_full)} samples, {len(set(subjects_full))} subjects")
    ...
    results = evaluate_extreme_models(X_train_full, y_train_full, subjects_full, cv_splits, device)
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            'best_accuracy': sorted_results[0][1]['mean_accuracy'] if sorted_results else None
        }, f, indent=2)
