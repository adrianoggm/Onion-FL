...
def plot_class_distribution(data_dir: str, label_strategy: str = "ordinal"):
    ...
    train_unique, train_counts = np.unique(dataset.train.y.astype(int), return_counts=True)
    ...
    fig.suptitle('SWEET Dataset - Class Distribution Analysis', fontsize=16, fontweight='bold')
    ...
    bars1 = ax1.bar(x - width, train_data, width, label='Train', color='#3498db', alpha=0.8)
    bars2 = ax1.bar(x, val_data, width, label='Val', color='#2ecc71', alpha=0.8)
    bars3 = ax1.bar(x + width, test_data, width, label='Test', color='#e74c3c', alpha=0.8)
    ...
    wedges, texts, autotexts = ax2.pie(
        train_data, 
        labels=[f'Class {cls}\n({train_data[i]} samples)' for i, cls in enumerate(unique_classes)],
        autopct='%1.1f%%',
    ...
    ax3 = axes[1, 0]
    total_data = [train_data[i] + val_data[i] + test_data[i] for i in range(len(unique_classes))]
    bars = ax3.bar(unique_classes, total_data, color=colors[:len(unique_classes)], alpha=0.8)
    ...
    print("=" * 80)
    print(f"Most common class: Class {unique_classes[train_data.index(max_class)]} ({max_class} samples)")
    print(f"Least common class: Class {unique_classes[train_data.index(min_class)]} ({min_class} samples)")
    ...
    print(f"   - Class 2 (High stress): original classes 3+4+5 = {train_data[2] + train_data[3] + train_data[4]} samples")
    print("=" * 80)
