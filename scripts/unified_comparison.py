...
colors = ['#e74c3c' if acc < 0.40 else '#f39c12' if acc < 0.48 else '#2ecc71' 
...
ax1.axvline(0.333, color='red', linestyle='--', alpha=0.5, linewidth=2, label='Random (33.3%)')
ax1.axvline(0.50, color='blue', linestyle='--', alpha=0.5, linewidth=2, label='50% Threshold')
...
for i, (bar, acc, std) in enumerate(zip(bars, accuracies, stds)):
    ax1.text(bar.get_width() + 0.005, bar.get_y() + bar.get_height()/2,
            f'{acc:.3f}±{std:.3f}', va='center', fontsize=8, fontweight='bold')
...
for i, (bar, f1) in enumerate(zip(bars2, f1_scores)):
    ax2.text(bar.get_width() + 0.005, bar.get_y() + bar.get_height()/2,
            f'{f1:.3f}', va='center', fontsize=8, fontweight='bold')
...
ax3.set_title(f'Best Model: {best_name.replace("_", " ")}\nAccuracy: {best_result["mean_accuracy"]:.1%}',
...
model_families = {
    'Tree-Based\n(GB, RF, DT)': ['Gradient_Boosting', 'Random_Forest', 'Decision_Tree', 'XGBoost', 'XGBoost_Optimized'],
    'Neural Networks\n(MLP, LSTM, GRU)': ['MLP_Small', 'MLP_Deep', 'MLP_Wide', 'LSTM_Attention', 'GRU_Attention', 'DeepMLP_Residual'],
    'Distance-Based\n(KNN, SVM)': ['KNN', 'SVM_RBF'],
...
bars_mean = ax4.bar(x - width/2, means, width, label='Mean', alpha=0.8, color='steelblue')
bars_max = ax4.bar(x + width/2, maxs, width, label='Best', alpha=0.8, color='forestgreen')
...
for rank, (name, res) in enumerate(sorted_results, 1):
    ...
    print(f"{rank:<5} {name.replace('_', ' '):<30} {acc_str:<18} {f1_str:<12} {family:<20}")

print("\n" + "=" * 90)
print("SUMMARY STATISTICS")
print("=" * 90)
print(f"Total models tested: {len(all_results)}")
print(f"Best accuracy: {sorted_results[0][1]['mean_accuracy']:.4f} ({sorted_results[0][0].replace('_', ' ')})")
print(f"Worst accuracy: {sorted_results[-1][1]['mean_accuracy']:.4f} ({sorted_results[-1][0].replace('_', ' ')})")
print(f"Mean accuracy: {np.mean([r[1]['mean_accuracy'] for r in sorted_results]):.4f}")
print(f"Median accuracy: {np.median([r[1]['mean_accuracy'] for r in sorted_results]):.4f}")
...
print(f"✅ Best model improves {(sorted_results[0][1]['mean_accuracy'] - 0.333) / 0.333 * 100:.1f}% over random (33.3%)")
