...
def analyze_dataset(data_dir: str, label_strategy: str = "ordinal"):
    ...
    # Basic stats
    print("\n1. Dataset Size:")
    print(f"   Train: {len(dataset.train.y)} samples, {len(dataset.train_subjects)} subjects")
    print(f"   Val:   {len(dataset.val.y)} samples, {len(dataset.val_subjects)} subjects")
    print(f"   Test:  {len(dataset.test.y)} samples, {len(dataset.test_subjects)} subjects")
