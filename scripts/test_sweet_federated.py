...
def test_sweet_loading():
    ...
    try:
        ...
        train_pct = len(dataset.train.y) / (len(dataset.train.y) + len(dataset.val.y) + len(dataset.test.y))
        val_pct = len(dataset.val.y) / (len(dataset.train.y) + len(dataset.val.y) + len(dataset.test.y))
        test_pct = len(dataset.test.y) / (len(dataset.train.y) + len(dataset.val.y) + len(dataset.test.y))
        
        print(f"✓ Dataset loaded successfully")
        print(f"  Train: {len(dataset.train_subjects)} subjects, {len(dataset.train.y)} samples ({train_pct:.1%})")
        print(f"  Val:   {len(dataset.val_subjects)} subjects, {len(dataset.val.y)} samples ({val_pct:.1%})")
        print(f"  Test:  {len(dataset.test_subjects)} subjects, {len(dataset.test.y)} samples ({test_pct:.1%})")
...
def main():
    ...
    if all_passed:
        ...
        print("     python scripts/prepare_sweet_baseline.py --data-dir data/SWEET/selection1")
        print("\n  2. Prepare federated splits:")
        print("     python scripts/prepare_sweet_federated.py --config configs/sweet_federated.example.yaml")
        print("\n  3. Run federated demo:")
        print("     python scripts/run_sweet_federated_demo.py --config configs/sweet_federated.example.yaml")
        return 0
    else:
