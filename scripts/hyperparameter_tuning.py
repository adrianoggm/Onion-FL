...
def main():
    parser = argparse.ArgumentParser(description="Hyperparameter tuning for SWEET models")
    parser.add_argument('--data-dir', type=str, required=True, help="Path to SWEET data")
    parser.add_argument('--label-strategy', type=str, default='ordinal_3class',
                       choices=['ordinal', 'ordinal_3class', 'binary'],
                       help="Label strategy")
    parser.add_argument('--output-dir', type=str, default='hypertuning_results',
    ...
    for rank, (name, result) in enumerate(sorted_results, 1):
        print(f"{rank}. {name}: {result['best_score']:.4f} ({result['tuning_time']/60:.1f} min)")
    ...
    for name, result in sorted_results:
        ...
        if base_name in baseline_scores:
            ...
            print(f"{symbol} {base_name}: {baseline:.4f} → {result['best_score']:.4f} ({improvement:+.2f}%)")
    ...
    summary = {
        ...
        'results': {name: {'best_score': res['best_score'], 
