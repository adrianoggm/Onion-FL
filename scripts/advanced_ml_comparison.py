...
def create_subject_level_cv_splits(
    subject_ids: np.ndarray, y_per_subject: dict, n_splits: int = 5, random_state: int = 42
):
    ...
    for train_subject_idx, test_subject_idx in skf.split(unique_subjects, subject_labels):
        ...
        cv_splits.append((
            np.where(train_sample_idx)[0],
            np.where(test_sample_idx)[0]
        ))
...
def plot_results(results_dict: dict, output_dir: Path):
    ...
    bars = ax1.barh(y_pos, mean_accs_sorted, xerr=std_accs_sorted, 
    ...
    for i, (bar, acc, std) in enumerate(zip(bars, mean_accs_sorted, std_accs_sorted)):
        ax1.text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                f'{acc:.3f}±{std:.3f}', va='center', fontsize=8)
    
    # F1 comparison
    ax2 = axes[0, 1]
    bars = ax2.barh(y_pos, mean_f1s_sorted, xerr=std_f1s_sorted,
                     color='forestgreen', alpha=0.7, capsize=5)
    ...
    for i, (bar, f1, std) in enumerate(zip(bars, mean_f1s_sorted, std_f1s_sorted)):
        ax2.text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                f'{f1:.3f}±{std:.3f}', va='center', fontsize=8)
    ...
    ax3.set_title(f'Confusion Matrix: {best_model.replace("_", " ")}', 
    ...
    bars1 = ax4.bar(x - width, metrics_data['Precision'], width, label='Precision', alpha=0.8)
    bars2 = ax4.bar(x, metrics_data['Recall'], width, label='Recall', alpha=0.8)
    bars3 = ax4.bar(x + width, metrics_data['F1-Score'], width, label='F1-Score', alpha=0.8)
    
    ax4.set_xlabel('Class', fontweight='bold')
    ax4.set_ylabel('Score', fontweight='bold')
    ax4.set_title(f'Per-Class Metrics: {best_model.replace("_", " ")}', 
                  fontweight='bold', fontsize=12)
    ax4.set_xticks(x)
    ...
    print(f"✅ All confusion matrices saved to: {output_dir / 'all_confusion_matrices.png'}")
...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(cv_splits, 1):
        ...
        print(f"  Fold {fold}: {train_subjs} train subjects, {test_subjs} test subjects")
    ...
    for rank, (model_name, results) in enumerate(sorted_models, 1):
        ...
        print(f"{rank:<5} {model_name.replace('_', ' '):<25} {acc_str:<20} {f1_str:<20}")
    ...
    print(f"{'=' * 80}")
    print(f"Mean Accuracy: {best_results['mean_accuracy']:.4f} ± {best_results['std_accuracy']:.4f}")
    print(f"Mean F1-Score: {best_results['mean_f1']:.4f} ± {best_results['std_f1']:.4f}")
    ...
    print(f"  - Improvement: {(best_results['mean_accuracy'] - 0.333) / 0.333 * 100:.1f}% over random")
