...
ax1.set_title(
    "TOP 20 MODELS - SWEET Stress Detection\n(Green=Tuned, Red=Extreme, Blue=Traditional)",
    fontweight="bold",
    fontsize=13,
)
ax1.axvline(
    0.50, color="gray", linestyle="--", alpha=0.5, linewidth=1.5, label="50% baseline"
)
ax1.axvline(
    0.55, color="gold", linestyle="--", alpha=0.7, linewidth=2, label="55% threshold"
)
...
categories = {
    "Traditional ML": [
        r for n, r in all_results.items() if n.startswith("Traditional")
    ],
    "Hyperparameter\nTuned": [
        r for n, r in all_results.items() if n.startswith("Tuned")
    ],
    "Extreme Deep\nArchitectures": [
        r for n, r in all_results.items() if n.startswith("Extreme")
    ],
}

cat_names = list(categories.keys())
means = [
    np.mean([r["mean_accuracy"] for r in cat_results])
    for cat_results in categories.values()
]
maxs = [
    np.max([r["mean_accuracy"] for r in cat_results])
    for cat_results in categories.values()
]
mins = [
    np.min([r["mean_accuracy"] for r in cat_results])
    for cat_results in categories.values()
]
...
trad_accs = [
    r["mean_accuracy"] for n, r in all_results.items() if n.startswith("Traditional")
]
tuned_accs = [
    r["mean_accuracy"] for n, r in all_results.items() if n.startswith("Tuned")
]
extreme_accs = [
    r["mean_accuracy"] for n, r in all_results.items() if n.startswith("Extreme")
]

data = [trad_accs, tuned_accs, extreme_accs]
bp = ax3.boxplot(
    data, labels=["Traditional", "Tuned", "Extreme"], patch_artist=True, showmeans=True
)
...
ax3.axhline(
    0.55, color="gold", linestyle="--", alpha=0.7, linewidth=2, label="55% target"
)
...
print(
    f"  - Traditional ML: {len([n for n in all_results if n.startswith('Traditional')])}"
)
print(
    f"  - Hyperparameter Tuned: {len([n for n in all_results if n.startswith('Tuned')])}"
)
