...
def load_labels():
    ...
    try:
        ...
        if (
            ...
            or "id" in df_labels.columns
        ):
            subj_col = [
                c
                for c in df_labels.columns
                if c.lower() in ["participant", "subject", "id", "dataset"]
            ][0]
        ...
        condition_cols = [
            c
            for c in df_labels.columns
            if "condition" in c.lower() or "stress" in c.lower() or "label" in c.lower()
        ]
        if condition_cols:
...
def create_visualizations(df_stats):
    ...
    fig.suptitle(
        "SWELL Dataset RAW - Análisis por Sujeto", fontsize=16, fontweight="bold"
    )

    # 1. Duración por sujeto
    ax = axes[0, 0]
    df_stats.plot(
        x="subject", y="duration_min", kind="bar", ax=ax, color="#3498db", legend=False
    )
    ...
    ax = axes[0, 1]
    df_stats.plot(
        x="subject", y="num_samples", kind="bar", ax=ax, color="#2ecc71", legend=False
    )
    ...
    ax = axes[0, 2]
    df_stats.plot(
        x="subject", y="rri_mean", kind="bar", ax=ax, color="#e74c3c", legend=False
    )
    ...
    ax = axes[1, 0]
    df_stats.plot(
        x="subject", y="hr_mean", kind="bar", ax=ax, color="#9b59b6", legend=False
    )
    ...
    ax = axes[1, 1]
    df_stats.plot(
        x="subject", y="rri_std", kind="bar", ax=ax, color="#f39c12", legend=False
    )
    ...
    ax = axes[1, 2]
    df_stats.plot(
        x="subject", y="outlier_pct", kind="bar", ax=ax, color="#e67e22", legend=False
    )
    ...
    ax = axes[0]
    ax.hist(
        df_stats["rri_mean"], bins=15, color="#3498db", alpha=0.7, edgecolor="black"
    )
...
def plot_example_signals():
    ...
    fig.suptitle(
        "SWELL Dataset RAW - Señales RRI de Ejemplo", fontsize=16, fontweight="bold"
    )
