...
def load_swell_data():
    ...
    features_cols = [
        col for col in df.columns if col not in ["condition", "split", "datasetId"]
    ]
...
def create_comprehensive_visualization():
    ...
    fig.suptitle(
        "SWELL Dataset - Comprehensive Overview", fontsize=16, fontweight="bold"
    )
    ...
    for idx, feature in enumerate(key_features, start=5):
        ...
        data_to_plot = [
            df[df["Stress_Label"] == "No Stress"][feature],
    ...
    feature_importance = pd.Series(stress_diff, index=top_features).sort_values(
        ascending=True
    )
    ...
    fig2.suptitle(
        "SWELL Dataset - Detailed Feature Analysis (Top 16 Features)",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for idx, feature in enumerate(top_6_features, start=1):
        ...
        # Histogram with KDE
        df[df["label"] == 0][feature].hist(
            bins=30, alpha=0.5, label="No Stress", color="#3498db", density=True, ax=ax
        )
        df[df["label"] == 1][feature].hist(
            bins=30, alpha=0.5, label="Stress", color="#e74c3c", density=True, ax=ax
        )
    ...
    ax2.bar(
        x + width / 2, stress_means, width, label="Stress", color="#e74c3c", alpha=0.8
    )
    ...
    split_stress_df = (
        df.groupby(["split", "Stress_Label"]).size().reset_index(name="count")
    )
    pivot_data = split_stress_df.pivot(
        index="split", columns="Stress_Label", values="count"
    )
